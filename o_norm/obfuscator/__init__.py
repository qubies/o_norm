...
import json
import random
from o_norm.resources import VIPER_DCES
...
leet_dict = {"a": ["@", "4", "^"], "b": ["6"], "c": ["("], "e": ["3"], "g": ["9", "6"], "h": [
...
def replace_QWERTYSMASH(word, local_probability=lambda: random.random()*ciel_prob*2):
    '''
...
class Obfuscator():
    ...
    def __init__(self, initial_probability=0.1, local_probability=None):
        ...
        if local_probability == None:
            ...
            self.methods = [
                partial(self.viper.obfuscate,
                        local_probability=local_probability),
                equal_spacing,
                partial(unequal_spacing,
                        local_probability=self.local_probability),
                partial(insert_punctuation,
                        local_probability=self.local_probability),
    ...
    def obfuscate(self, word):
        ...
        '''
        return probability_wrapper(initial_probability=self.initial_probability)(random.choice(self.methods))(word)
