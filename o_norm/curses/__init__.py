from o_norm.preprocessing import purify
...
class example_generator:
    def __init__(self, curse_dict, vocabulary, max_length=50):
        ...
        self.really_common_curses = [
            x.lower() for x in curse_dict.keys() if curse_dict[x] > 1
        ]
    ...
    def create_curse_examples(self, number_of_examples):
        return create_from_fn(
            "Full Curse Dictionary",
            self.get_random_curse,
            number_of_examples,
            self.max_length,
    ...
    def create_common_curse_examples(self, number_of_examples):
        return create_from_fn(
            "Common Curse Dictionary",
            self.get_random_common_curse,
            number_of_examples,
            self.max_length,
            contains_curses=True,
        )
    ...
    def create_really_common_curse_examples(self, number_of_examples):
        return create_from_fn(
            "Really Common Curse List",
            self.get_random_really_common_curse,
            number_of_examples,
            self.max_length,
            contains_curses=True,
        )
    ...
    def create_short_word_examples(self, number_of_examples):
        return create_from_fn(
            "Short Good Word Dictionary",
            self.get_random_short_word,
            number_of_examples,
            self.max_length,
            contains_curses=False,
        )
...
def create_from_fn(
    name, fn, number_of_examples, max_length, contains_curses=True, obfus=True
):
    examples = []
    ys = []
    global e_dup
...
if __name__ == "__main__":
    test = example_generator(RESOURCES["CURSES"], RESOURCES["VOCABULARY"])
